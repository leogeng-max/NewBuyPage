...
- **Follow-up:** the loop/repeat glyph was swapped for a plain radio button (same ring-and-dot treatment as the Classification/Role option lists) — filled when on, hollow ring when off. Everything else (position next to the title, title dim/bright sync, tap-to-toggle, transient label) unchanged. Concept F in the exploration screen keeps the original loop icon, since that concept's whole point is showing that specific alternative.
- **Follow-up:** tried moving it into the topbar's top-right corner (next to the Customize icon) — wrong read; clarified the intent was the top-right corner of the *title block* itself (above the ATZ/price line), not the topbar. Moved it there instead: `.memo-title-block` is now `position:relative`, the toggle sits `position:absolute; top:0; right:0;` inside it. Also made the title-dim lookup more robust while making this change — it now finds `.memo-title` via `closest(".memo-screen")` instead of an immediate-parent lookup, so repositioning it again later won't silently break that link.
- **Follow-up (sizing/spacing pass):** radio went 12px → 10.5px → 9px, settling smaller than the first "title-adjacent" version. Confirmed via computed styles each time that it's a genuine `width:9px; height:9px; border-radius:50%` circle (checked `getBoundingClientRect` and `getComputedStyle` directly) — the earlier "looks square" read was a small-size legibility issue, not a rendering bug. Also increased the gap above the ATZ/price line (`margin-bottom` on the toggle, 5px → 10px) so it doesn't crowd the ticker text.
- **Follow-up (another pass):** dropped the title-dim behavior entirely — tapping the toggle no longer changes the title's color, only the radio + its momentary label. This actually brings the real implementation back in line with what Concept I's own description always said ("unlike E, the title stays neutral") — the title-linkage had drifted in from E during implementation and wasn't really part of I to begin with. Also: swapped the absolute top-right-of-block positioning for an in-flow placement directly above the ATZ/price line, label-then-radio (radio right-aligned to match ATZ, so the label appearing/disappearing never shifts the radio's position), and changed `.memo-title-row` from `align-items:baseline` to `flex-start` so the radio's top edge lines up with the title's top edge. Bumped the radio back up to 12px (from 10px) — too small to read clearly as round.
